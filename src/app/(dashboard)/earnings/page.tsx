...
export default function EarningsPage() {
    ...
    // Tracking state
    const [trackingTicker, setTrackingTicker] = useState<string | null>(null);
    ...
    const fetchMonitors = useCallback(async () => {
        if (!userEmail) return;
        try {
            const res = await fetch(`${config.apiUrl}/earnings/monitors?user_email=${encodeURIComponent(userEmail)}`);
            if (res.ok) {
                const data = await res.json();
                setActiveMonitors(data.monitors.map((m: any) => m.ticker));
            }
        } catch (err) {
            console.error(err);
        }
    ...
    }, [fetchEarnings, fetchMonitors]);

    const handleTrack = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!trackingTicker || !email) return;
        
        setTrackLoading(true);
        try {
            const res = await fetch(`${config.apiUrl}/earnings/start-monitor`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ ticker: trackingTicker, user_email: email })
            });
            const data = await res.json();
            
            if (res.ok) {
                setTrackSuccess(trackingTicker);
                setActiveMonitors(prev => [...prev, trackingTicker]);
                setTimeout(() => {
                    setTrackSuccess(null);
                    setTrackingTicker(null);
                    setEmail("");
                }, 3000);
            } else {
                alert(`Error: ${data.detail || 'Failed to start monitor'}`);
            }
        } catch (err) {
    ...
    return (
        <>
            ...
            {activeMonitors.length > 0 && (
                <div className="mb-6 animate-pulse border border-profit/20 bg-profit/5 rounded-xl px-5 py-3 flex items-center gap-4">
                    <span className="relative flex h-3 w-3">
                      <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-profit opacity-75"></span>
                      <span className="relative inline-flex rounded-full h-3 w-3 bg-profit"></span>
                    </span>
                    <p className="text-sm font-semibold text-text-main flex-1">
                        Offline Monitoring Active: <span className="text-primary ml-1">{activeMonitors.join(", ")}</span>
                    </p>
                    <span className="text-xs text-text-muted hidden sm:block">We will email you instantly when earnings print.</span>
                </div>
            )}
            ...
            {trackingTicker && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm">
                    <motion.div 
                        ...
                        className="bg-surface border border-border p-6 rounded-xl shadow-2xl w-full max-w-sm"
                    >
                        {trackSuccess === trackingTicker ? (
                            <div className="text-center py-4 space-y-3">
                                <div className="w-12 h-12 rounded-full bg-profit/20 text-profit flex items-center justify-center mx-auto mb-2">
                                    <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
                                </div>
                                <h3 className="text-lg font-bold text-text-main">Tracker Activated</h3>
                                <p className="text-xs text-text-muted">You will receive an email as soon as {trackingTicker} earnings drop.</p>
                            </div>
                        ) : (
                            <form onSubmit={handleTrack} className="space-y-4">
                                <div>
                                    <h3 className="text-lg font-bold text-text-main mb-1">Track {trackingTicker}</h3>
                                    <p className="text-xs text-text-muted">Enter email to receive offline alerts.</p>
                                </div>
                                <div>
                                    <input 
                                        type="email" 
                                ...
                                <div className="flex gap-2 justify-end mt-4">
                                    <button 
                                        type="button" 
                                        onClick={() => setTrackingTicker(null)}
                                        disabled={trackLoading}
                                        className="px-4 py-2 text-xs font-semibold rounded-lg hover:bg-white/5 text-text-muted transition-colors disabled:opacity-50"
                                    >
                                        Cancel
                                    </button>
                                    <button 
                                        type="submit" 
                                        disabled={trackLoading || !email}
                                        className="px-4 py-2 text-xs font-semibold rounded-lg bg-primary text-primary-foreground hover:bg-primary/90 transition-colors disabled:opacity-50 flex items-center gap-2"
                                    >
                                        {trackLoading && <span className="w-3 h-3 border-2 border-white border-t-transparent rounded-full animate-spin" />}
                                        Start Monitor
                                    </button>
                                </div>
                            </form>
                                        ...
                                        <div className="mt-1 space-y-0.5 max-h-[80px] overflow-y-auto">
                                            {cell.earnings.slice(0, 4).map((ev, j) => (
                                                <div key={j} className="px-1.5 py-0.5 text-[10px] rounded bg-primary-dim/50 text-primary border border-primary/20 truncate">
                                                    ...
                                                    {activeMonitors.includes(ev.ticker) ? (
                                                        <span className="ml-2 px-1 rounded bg-profit/20 text-profit float-right text-[9px] uppercase font-bold">
                                                            Active
                                                        </span>
                                                    ) : (
                                                        <button 
                                                            onClick={(e) => { e.stopPropagation(); setTrackingTicker(ev.ticker); }}
                                                            className="ml-2 px-1 rounded bg-primary text-primary-foreground opacity-0 group-hover:opacity-100 transition-opacity float-right hover:bg-white"
                                                        >
                                                            Track
                                                        </button>
                                                    )}
                            ...
                            {sortedEvents.slice(0, 100).map((ev, i) => (
                                ...
                                >
                                    ...
                                    <td className="px-4 py-3 text-right">
                                        {activeMonitors.includes(ev.ticker) ? (
                                            <span className="px-2 py-1 text-[10px] font-bold uppercase tracking-wider bg-profit/10 text-profit rounded border border-profit/20">
                                                Monitoring
                                            </span>
                                        ) : (
                                            <button 
                                                onClick={() => setTrackingTicker(ev.ticker)}
                                                className="px-2 py-1 text-[10px] font-bold uppercase tracking-wider bg-primary/10 text-primary hover:bg-primary hover:text-primary-foreground rounded transition-colors"
                                            >
                                                Track
                                            </button>
                                        )}
