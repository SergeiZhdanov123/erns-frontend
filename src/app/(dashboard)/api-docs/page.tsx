...
console.log(data);`,
  ...
  "earnings_forecast": { "eps_current_year": 7.1, "eps_next_year": 7.8, "eps_growth_pct": 9.8 }
            ...
            },
            {
                ...
                params: [
                    { name: "ticker", type: "query", description: "Stock ticker", required: true },
                ],
                responseExample: `{
  "ticker": "AAPL",
  "event_date": "2025-01-30",
  ...
  "confidence": "medium"
                ...
                curl: `curl "${BASE}/earnings/live-monitor?ticker=AAPL"`,
                python: `r = requests.get("${BASE}/earnings/live-monitor", params={"ticker": "AAPL"})`,
                javascript: `const res = await fetch("${BASE}/earnings/live-monitor?ticker=AAPL");`,
            },
            {
                method: "GET",
                path: "/earnings/surprises",
                description: "Historical EPS surprises returning detailed quarterly beat/miss magnitudes.",
                params: [
                    { name: "tickers", type: "query", description: "Comma-separated tickers", required: true },
                ],
                responseExample: `{ "AAPL": { "surprise_pct": 5.4, "status": "beat" } }`,
                curl: `curl "${BASE}/earnings/surprises?tickers=AAPL"`,
                python: `r = requests.get("${BASE}/earnings/surprises", params={"tickers": "AAPL"})`,
                javascript: `const res = await fetch("${BASE}/earnings/surprises?tickers=AAPL");`,
            },
            {
                method: "GET",
                path: "/earnings/movers",
                description: "Top post-earnings stock movers highlighting maximum volatility and post-release volume spikes across the market.",
                params: [],
                responseExample: `[ { "ticker": "TSLA", "move_pct": 14.5, "direction": "up" } ]`,
                curl: `curl "${BASE}/earnings/movers"`,
                python: `r = requests.get("${BASE}/earnings/movers")`,
                javascript: `const res = await fetch("${BASE}/earnings/movers");`,
            },
            {
                method: "GET",
                path: "/earnings/guidance",
                description: "Extraction of structured corporate forward guidance and forward-looking text markers.",
                params: [
                    { name: "tickers", type: "query", description: "Comma-separated tickers", required: true },
                ],
                responseExample: `{ "AAPL": [ "We expect revenue to grow double digits next quarter." ] }`,
                curl: `curl "${BASE}/earnings/guidance?tickers=AAPL"`,
                python: `r = requests.get("${BASE}/earnings/guidance", params={"tickers": "AAPL"})`,
                javascript: `const res = await fetch("${BASE}/earnings/guidance?tickers=AAPL");`,
            },
            {
                method: "GET",
                path: "/earnings/sentiment-batch",
                description: "Natural language contextual sentiment aggregation mapped against transcript passages. Generates polar magnitude indexing.",
                params: [
                    { name: "tickers", type: "query", description: "Comma-separated tickers", required: true },
                ],
                responseExample: `{ "AAPL": { "score": 0.82, "label": "bullish" } }`,
                curl: `curl "${BASE}/earnings/sentiment-batch?tickers=AAPL"`,
                python: `r = requests.get("${BASE}/earnings/sentiment-batch", params={"tickers": "AAPL"})`,
                javascript: `const res = await fetch("${BASE}/earnings/sentiment-batch?tickers=AAPL");`,
            },
            {
                method: "GET",
                path: "/earnings/analyst-leaderboard",
                description: "Cross reference of historically accurate Wall Street analyst predictions linked toward institutional profiles.",
                params: [
                    { name: "ticker", type: "query", description: "Stock ticker", required: true },
                ],
                responseExample: `[ { "analyst": "Dan Ives", "firm": "Wedbush", "accuracy_score": 92.5 } ]`,
                curl: `curl "${BASE}/earnings/analyst-leaderboard?ticker=AAPL"`,
                python: `r = requests.get("${BASE}/earnings/analyst-leaderboard", params={"ticker": "AAPL"})`,
                javascript: `const res = await fetch("${BASE}/earnings/analyst-leaderboard?ticker=AAPL");`,
            },
            {
                method: "GET",
                path: "/market/anomalies",
                description: "AI-driven detection grid filtering systemic outliers across intraday volumes vs earning beats frameworks.",
                params: [
                     { name: "tickers", type: "query", description: "Comma-separated tickers", required: true },
                ],
                responseExample: `[ { "ticker": "MSFT", "anomaly_type": "volume_spike", "severity": 8.5 } ]`,
                curl: `curl "${BASE}/market/anomalies?tickers=MSFT"`,
                python: `r = requests.get("${BASE}/market/anomalies", params={"tickers": "MSFT"})`,
                javascript: `const res = await fetch("${BASE}/market/anomalies?tickers=MSFT");`,
            },
            {
                method: "GET",
                path: "/company/{ticker}/capital-allocation",
                description: "Quantified flow map isolating Free Cash Flow diversions toward repurchases, dividends, CapEx, or debt reduction strategies.",
                params: [
                     { name: "ticker", type: "path", description: "Stock ticker", required: true },
                ],
                responseExample: `{ "operating_cash": 1200000, "capex": 200000, "dividends": 100000, "buybacks": 50000 }`,
                curl: `curl "${BASE}/company/AAPL/capital-allocation"`,
                python: `r = requests.get("${BASE}/company/AAPL/capital-allocation")`,
                javascript: `const res = await fetch("${BASE}/company/AAPL/capital-allocation");`,
            },
            {
                method: "GET",
                            ...
                            <div className="mt-8 bg-surface/80 border border-border rounded-xl p-5">
                                ...
                                <table className="w-full text-sm">
                                    ...
                                    <tbody className="text-text-main font-mono">
                                        ...
                                        <tr className="border-t border-border/50"><td className="py-2">Starter</td><td>0</td><td className="text-text-muted text-xs font-sans">No API access</td></tr>
                                        <tr className="border-t border-border/50"><td className="py-2">Pro</td><td>120</td><td className="text-text-muted text-xs font-sans">API key required</td></tr>
