...
export default function LiveEarningsPage() {
    ...
    useEffect(() => {
        try {
            const saved = localStorage.getItem("tyche-live-earnings");
            if (saved) {
                const parsed = JSON.parse(saved);
                if (Array.isArray(parsed)) {
                    const initialSlots = parsed.map(item => {
                        if (typeof item === 'string') {
                            // Legacy format migrations
                            return { ticker: item, result: null, polling: true };
                        }
                        // Hydrate full cached data instantly to prevent UI flicker
                        if (item.result?.status === 'dropped') {
                            // Do not resume polling for dropped tickers
                            return { ...item, polling: false };
                        }
                        return { ...item, polling: true };
                    });
    ...
    // Save full detailed tracking slots to localStorage to avoid blank loaders
    useEffect(() => {
        if (!isHydrated) return;
        localStorage.setItem("tyche-live-earnings", JSON.stringify(slots));
    }, [slots, isHydrated]);

    // Restart polling for hydrated slots
    useEffect(() => {
        if (!isHydrated || !isPaid) return;
        slots.forEach(s => {
            if (!intervalsRef.current.has(s.ticker)) {
                pollTicker(s.ticker);
                const interval = setInterval(() => pollTicker(s.ticker), pollInterval);
                intervalsRef.current.set(s.ticker, interval);
            }
        });
    }, [isHydrated, isPaid, slots, pollInterval]);
        ...
        try {
            ...
            setSlots(prev => {
                const updated = prev.map(s => {
                    if (s.ticker === ticker) {
                        // If status changed from waiting to dropped, play sound + send notification
                        if (s.result?.status === "waiting" && data.status === "dropped") {
                            if (soundEnabled && audioRef.current) {
                                audioRef.current.play().catch(() => { });
                            }
                            // Email and DB Notification are now authoritatively handled by the background worker.py
                            // when this exact state transition triggers in MongoDB. We only play the client sound here.
                        }
