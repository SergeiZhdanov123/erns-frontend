...
// Custom hook for AI Overview
function useAIOverview(ticker: string | undefined) {
    const [overview, setOverview] = useState<string | null>(null);
    const [loading, setLoading] = useState(false);

    useEffect(() => {
        if (!ticker) { setOverview(null); return; }
        setLoading(true);
        fetch(`${config.apiUrl}/company/${ticker}/ai-overview`)
            .then(res => res.json())
            .then(data => setOverview(data.overview_markdown))
...
export default function ChartPage() {
    ...
    const { overview: aiOverview, loading: aiLoading } = useAIOverview(activeTicker || undefined);
                    ...
                    <>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                            <div className="col-span-1 lg:col-span-3 flex flex-col gap-6">
                                ...
                                {/* AI Overview Section */}
                                <div className="bg-surface/50 border border-white/5 rounded-2xl p-5 shadow-sm text-sm text-text-main mt-2">
                                    <div className="flex items-center gap-3 mb-4">
                                        <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-emerald-500 to-emerald-700 flex items-center justify-center shrink-0">
                                            <svg className="w-4 h-4 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09z" />
                                            </svg>
                                        </div>
                                        <div>
                                            <h3 className="font-bold text-text-main">AI Overview</h3>
                                            <p className="text-[10px] text-text-muted">Generated by Erns Intelligence</p>
                                        </div>
                                    </div>
                                    
                                    {aiLoading ? (
                                        <div className="flex flex-col gap-2">
                                            <div className="animate-pulse bg-white/5 h-4 w-3/4 rounded-lg"></div>
                                            <div className="animate-pulse bg-white/5 h-4 w-full rounded-lg"></div>
                                            <div className="animate-pulse bg-white/5 h-4 w-5/6 rounded-lg"></div>
                                        </div>
                                    ) : aiOverview ? (
                                        <div className="prose prose-invert prose-sm max-w-none prose-h3:text-primary prose-h3:text-sm prose-h3:font-bold prose-h3:mt-3 prose-h3:mb-1 prose-p:leading-relaxed prose-li:my-0">
                                            <ReactMarkdown>{aiOverview}</ReactMarkdown>
                                        </div>
                                    ) : (
                                        <div className="text-text-muted text-xs italic">
                                            No AI overview available for this ticker.
                                        </div>
                                    )}
                                </div>
                            </div>
