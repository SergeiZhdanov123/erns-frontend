...
function renderMarkdown(content: string): React.ReactNode {
    ...
    const flushParagraph = () => {
        if (currentParagraph.length > 0) {
            ...
            if (text) {
                elements.push(
                    <p key={`p-${elements.length}`} className="text-sm my-1.5 text-text-muted leading-relaxed">
                        {mdInline(text)}
                    </p>
                );
            }
    ...
    const flushList = () => {
        if (currentList) {
            if (currentList.type === "ul") {
                elements.push(
                    <ul key={`ul-${elements.length}`} className="space-y-1 my-1.5 ml-3">
                ...
                elements.push(
                    <ol key={`ol-${elements.length}`} className="space-y-1 my-1.5 ml-3">
                        {currentList.items.map((item, ii) => (
                            <li key={ii} className="flex items-start gap-2 text-sm text-text-muted">
    ...
    for (const line of lines) {
        ...
        if (hm) {
            ...
            const cls = level === 1
                ? "text-base font-bold text-text-main mt-5 mb-1"
                : level === 2
                    ? "text-sm font-bold text-text-main mt-4 mb-1"
                    : "text-sm font-semibold text-primary mt-3 mb-0.5";
            elements.push(<div key={`h-${elements.length}`} className={cls}>{mdInline(hm[2])}</div>);
            continue;
        }
