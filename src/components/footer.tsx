...
export function Footer() {
  return (
    <footer className="relative py-16 md:py-20 px-6 border-t border-white/[0.04] bg-background overflow-hidden">
      {/* Subtle top glow */}
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-12 mb-12 md:mb-16">
          ...
          {footerColumns.map((col, i) => (
            <div key={i}>
              <h4 className="font-semibold text-text-main mb-4 md:mb-5 text-sm">{col.title}</h4>
              <ul className="space-y-2.5">
                {col.links.map((link, j) => (
                  <li key={j}>
                    <Link href={link.href}
                      className="group relative text-white/30 text-sm hover:text-primary transition-colors duration-200 inline-block"
                    >
                      {link.label}
          ...
          <div className="flex gap-5">
            {socialLinks.map((social, i) => (
              <a key={i} href={social.href} target="_blank" rel="noopener noreferrer" aria-label={social.label}
                className="text-white/20 hover:text-primary hover:scale-110 transition-all duration-200"
              >
